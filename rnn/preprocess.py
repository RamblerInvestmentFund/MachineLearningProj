...
def get(stock):
    ...
    print(f"\ndownloading {stock}")
    return yf.download(tickers=stock)
...
def rnn_data_pipeline(stocks=snp(), timesteps=50):
    ...
    df = [get(stock) for stock in stocks]
    df = pd.concat(df)
